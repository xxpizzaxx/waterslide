...
class WaterslideServer(hostname: String,
    ...
    time
      ...
      .flatMap {
        case (_, body) =>
          ...
          Try { OM.readTree(body) }.toOption
            .map(Process.emit)
            .getOrElse(Process.empty)
      }
  )
  ...
  topic.subscribe
    .map(x => lastValidResponse = x)
    .run
    .runAsync(
      f =>
        f.leftMap(t =>
            log.error(t)("failed to update the lastValidResponse cache"))
          .rightMap(_ =>
            log.error("lastValid Response cache updater exited with Unit")))
  ...
  val server = BlazeBuilder
    .bindHttp(host = hostname, port = port)
    .withWebSockets(true)
    .mountService(route, "/")
    .start
